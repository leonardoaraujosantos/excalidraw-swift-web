...
> **On-device benchmark (`ExcalidrawAppBenchmarks/RendererComparisonBenchmark`, iPad).** A head-to-head CPU-vs-GPU stress test with a per-phase breakdown. Building it surfaced and fixed three real issues in the GPU path: (1) a per-emit `reserveCapacity` that defeated `Array`'s geometric growth and made geometry build **O(n²)** (~8 s for 200 shapes); (2) excessive triangle count — a round disc at *every* stroke vertex — cut ~5.5× by angle-thresholded joins + endpoint-only caps + lighter Bézier flattening; (3) full re-tessellation every frame, fixed with a per-element `GeometryCache` (cold→warm rebuild 2230 ms→36 ms for 1500 shapes). Net effect on device: Metal went from ~35 s/frame to ~33–57 ms/frame. Steady-state (cached) reads, 1200×800: shapes-only n=1500 `cpu=15 ms metal=34 ms`; mixed n=500 `cpu=31 ms metal=36 ms`; mixed n=1500 `cpu=36 ms metal=57 ms`. **Honest finding:** the GPU rasterization is cheap (8–16 ms even at 1500 shapes) and scales well, but on Apple-Silicon hardware CoreGraphics is already fast, and the Metal path still pays readback/compositing overhead plus a CG overlay pass for non-tessellated content (freedraw/text) — so Metal is *competitive* (within ~1.5–2×) but not a clear win for full-repaint frames. It earns its keep where the CPU renderer is the bottleneck and as a runtime A/B comparison. Regression tests guard the O(n²) build and the cache behavior.
