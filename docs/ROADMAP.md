...
> **Status: complete (core).** Real-time iPad ↔ browser collaboration is delivered. A shared, byte-identical wire protocol is spoken by both clients — Swift `ExcalidrawCollab` (`CollabMessage`, `Reconcile`, `CollabCodec`, `CollabClient`) and TypeScript `@cyberdynecorp/excalidraw-svelte/protocol` + `@cyberdynecorp/excalidraw-svelte` `CollabSession` — locked by the `Fixtures/protocol/*.json` corpus both test suites assert against. A Node relay (`web/server/`) handles rooms, presence, and an in-memory scene snapshot; the existing `version`/`versionNonce` reconciliation resolves concurrent edits identically on both sides. `EditorModel` joins a room (broadcast local edits, apply remote reconciled without polluting undo, presence/cursors, auto-reconnect). **Validated end-to-end:** the real SwiftUI app in an iOS simulator and a real browser join one room and converge live — automated by `web/scripts/collab-live.sh` (XCUITest `CollabLiveUITests` + Playwright). The TypeScript + Svelte 5 twin (the web client) is its phase T7 — see **[docs/TYPESCRIPT_SVELTE_PORT.md](TYPESCRIPT_SVELTE_PORT.md)**.
> **Deferred:** durable/Redis relay persistence (snapshot is in-memory), end-to-end encryption, and follow mode.
>
> **Optional Yjs/CRDT backend (web).** `@cyberdynecorp/excalidraw-yjs` adds an alternative, additive web collaboration engine: it two-way binds the `EditorStore` to a Yjs `Y.Doc` (id-keyed per-element `Y.Map`), unlocking the Yjs provider ecosystem (y-websocket, Hocuspocus, custom gateways) and **field-level merge** for concurrent same-element edits. The element-LWW engine above stays canonical and shared with Swift; the Yjs path is a *parallel* engine that bypasses `reconcileElements`, the core takes no `yjs` dependency, and a Yjs-synced scene still round-trips `.excalidraw` and interoperates with LWW. Web-only by design (the Swift twin stays pure-Swift). See **[collaboration-yjs](../openspec/specs/collaboration-yjs/spec.md)** and `web/packages/excalidraw-yjs/`. (issue #11; downstream: CyberdyneCorp/cyberspace.)
